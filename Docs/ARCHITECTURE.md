...
- Fever-end presentation consumes the committed `ObstacleResolutionResult` as an ordered announcement → exact-cell telegraph → gold wave → removal → gravity → refill sequence. The view never selects effect cells. The existing serialized reconfiguration overlay is mode-styled gold for Fever and blue with a `SHUFFLE` label for deadlock recovery; Reduced Motion retains text and thick cell markers while suppressing scaling and the animated wave.
