...
namespace MathGame.BoardResolution
{
    ...
    {
        internal ObstacleResolutionResult(ObstacleResolutionFailure failure, DomainBoard board, ObstacleResolutionMode mode, FeverEndPattern pattern, BoardSystemEffectId effectId, int nextId, List<RemovedNumberDelta> selected, List<RemovedNumberDelta> collateral, List<MovedBlockDelta> moved, List<SpawnedBlockDelta> spawned, List<ObstacleDamageDelta> damage, List<ObstacleDestroyedEvidence> destroyed) { Failure = failure; Board = board; Mode = mode; Pattern = pattern; SystemEffectId = effectId; NextBlockIdValue = nextId; SelectedRemoved = Array.AsReadOnly(selected.ToArray()); CollateralRemoved = Array.AsReadOnly(collateral.ToArray()); var all = new List<RemovedNumberDelta>(selected); all.AddRange(collateral); Removed = Array.AsReadOnly(all.ToArray()); Moved = Array.AsReadOnly(moved.ToArray()); Spawned = Array.AsReadOnly(spawned.ToArray()); ObstacleDamage = Array.AsReadOnly(damage.ToArray()); DestroyedObstacles = Array.AsReadOnly(destroyed.ToArray()); }
        public bool Succeeded => Failure == ObstacleResolutionFailure.None; public ObstacleResolutionFailure Failure { get; } public DomainBoard Board { get; } public ObstacleResolutionMode Mode { get; } public FeverEndPattern Pattern { get; } public BoardSystemEffectId SystemEffectId { get; } public int NextBlockIdValue { get; }
        ...
        internal static ObstacleResolutionResult Fail(ObstacleResolutionFailure f) => new ObstacleResolutionResult(f, null, default, default, default, 0, new List<RemovedNumberDelta>(), new List<RemovedNumberDelta>(), new List<MovedBlockDelta>(), new List<SpawnedBlockDelta>(), new List<ObstacleDamageDelta>(), new List<ObstacleDestroyedEvidence>());
    }
}
