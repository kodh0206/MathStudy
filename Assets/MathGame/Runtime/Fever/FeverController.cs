...
namespace MathGame.Fever
{
    public sealed class FeverController : IDisposable
    {
        ...
        { if (State != FeverState.Charging) return FeverChargeApplyResult.NotCharging; var applied = charge.ApplyNormalAttempt(result); BumpPresentationRevision(); if (applied == FeverChargeApplyResult.ReachedMaximum) { charge.Charging = false; State = FeverState.PendingEntry; } if (result != null && result.After != null && result.After.Status != StageSessionStatus.Active) Abort(result.After.Status == StageSessionStatus.Success ? FeverTerminationReason.StageSucceeded : FeverTerminationReason.StageFailed); return applied; }
